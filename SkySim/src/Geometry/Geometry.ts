...
export class Triangle {
     ...
     */
    public constructor(a: Array<number>, b: Array<number>, c: Array<number>) {
        this.a = a;
        this.b = b;
        this.c = c;
    }
...
export class Circle {
    ...
    public constructor(radius: number, n_phi: number) {
        this.radius = radius;
        this.n_phi = n_phi;
    }

    public getTriangleSphericalIndices(index_phi: number) {
    ...
    public getArrayBufferContent(): Float32Array {
        let triangles = [];
        ...
        let trianglePoints = [];
        triangles.forEach((item: Triangle) => {
...
export class Sphere {
     ...
     */
    public constructor(radius: number, n_theta: number, n_phi: number) {
        this.radius = radius;
     ...
     */
    public getTriangleSphericalIndices(index_theta: number, index_phi: number, index_down: boolean): Triangle {
     ...
     * @returns {Triangle} The requested triangle.
     */
    public getTriangleIndex(index: number): Triangle {
    ...
    public getSphericalTriangleFromIndices(index_theta, index_phi, index_down) {
        // Values of theta and phi represent the corner of the wanted triangle with the smallest values of theta and phi
        let phi = (index_phi % this.n_phi) * 2 * Math.PI / this.n_phi;
        let theta = (index_theta % this.n_theta) * Math.PI / this.n_theta;
    ...
    public getCartesianTriangleFromIndices(index_theta, index_phi, index_down) {
        // Values of theta and phi represent the corner of the wanted triangle with the smallest values of theta and phi
    ...
    public getTextureCoordinateBufferContent() {
        // For the first and last theta_index only calculate one triangle per phi_index (Simply leave out the index_down variation)
        let triangles = [];
        for(let i_theta = 0; i_theta < this.n_theta; i_theta++) {
            for(let i_phi = 0; i_phi < this.n_phi; i_phi++) {
 ...
 */
export function sphericalToCartesian(radius: number, theta: number, phi: number): Array<number> {
    return Array(
...
export function circularToCartesian(radius: number, phi: number): Array<number> {
    return Array(
        radius * Math.cos(phi),
        radius * Math.sin(phi),
        0.0
